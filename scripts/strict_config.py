...
def main():
    ...
    try:
        ...
        custom_user_js = """
        // Your custom user.js content here
        """
        ...
        if create_policies_json(policies_dir) and create_distribution_ini(firefox_dir):
            ...
            print("DuckDuckGo has been set as the default search engine.")
...
if __name__ == "__main__":
    try:
        if 'close_original' in sys.argv:
            sys.exit(0)

        if platform.system() == "Windows" and not is_admin():
            if 'asadmin' not in sys.argv:
                script = os.path.abspath(sys.argv[0])
                params = ' '.join([script] + sys.argv[1:] + ['asadmin', 'close_original'])
